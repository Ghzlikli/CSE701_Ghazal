...
Genetic Algorithm is an effort at mimicking biology and is inspired by chromosomes' behavior. Every chromosome comprises several genes defining the characteristics of the entire chromosome. Moreover,  more robust characteristics are more likely to be inherited by the next generation.
...
1. Crossover (recombine): Pairs of parent chromosomes produce the next generation of chromosomes, known as children or offspring chromosomes. There are plenty of approaches for the crossover, among which we use the single-point method.
...
- `ReadCSV.hpp`: Contains the `csv` class for reading the input file in csv format.
